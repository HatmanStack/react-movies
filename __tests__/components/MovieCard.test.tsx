...
describe("MovieCard", () => {
  ...
  it("is exported as a memoized component", () => {
    // MovieCard is wrapped in React.memo at the module level
    expect(MovieCard).toHaveProperty("$$typeof", Symbol.for("react.memo"));
  });
  ...
  it("renders correct poster URL via TMDbService", () => {
    ...
    expect(poster.props.source.uri).toContain("/test.jpg");
    expect(poster.props.source.uri).toContain("image.tmdb.org");
  });
});
