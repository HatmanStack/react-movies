...
jest.mock("expo-sqlite", () => {
  ...
  class MockDatabase {
    async execAsync(sql) {
      if (sql.includes("CREATE TABLE IF NOT EXISTS movie_details"))
        tables.set("movie_details", []);
    ...
    async runAsync(sql, params = []) {
      if (sql.includes("INSERT OR REPLACE INTO movie_details")) {
        ...
        const [id, image_url, iso_639_1, iso_3166_1, key, site, size, type] =
          params;
        table.push({
    ...
    async getAllAsync(sql, params = []) {
      if (sql.includes("FROM movie_details")) {
        ...
        if (sql.includes("WHERE favorite = 1"))
          return table.filter((row) => row.favorite === 1);
        if (sql.includes("WHERE popular = 1"))
          return table.filter((row) => row.popular === 1);
        if (sql.includes("WHERE toprated = 1"))
          return table.filter((row) => row.toprated === 1);
        return table;
        ...
        if (sql.includes("WHERE type = ?") && sql.includes("AND id = ?")) {
          return table.filter(
            (row) => row.type === params[0] && row.id === params[1],
          );
        }
        if (sql.includes("WHERE id = ?"))
          return table.filter((row) => row.id === params[0]);
